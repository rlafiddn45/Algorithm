...
class Solution:
    def splitArray(self, nums: List[int], m: int) -> int:
        ...
        while m - 1:
            m -= 1  # 횟수가 하나씩 줄어듦
            idx = 0  # 기준이 되는 인덱스
            diff = sum(nums)  # 처음 기준 양 옆 간의 숫자의 합의 차이는 (전체 합 - 0)
            for i in range(len(nums)):
                after_diff = diff - nums[i] * 2  # 원소를 하나 추가하면 2 * 원소만큼 빠진다
                if diff > abs(after_diff):
                    diff = after_diff
                else:
                    idx = i
                    break
            # print('idx :', idx)
            arrays.append(nums[:idx])
            arrays.append(nums[idx:])
            arrays.sort(key=lambda x: sum(x))
            # print('arrays :', arrays)
            nums = arrays.pop()
            print('nums :', nums)
        arrays.append(nums)
